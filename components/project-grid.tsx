    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-12">
      {projects.map((project) => (
        <div
          ...
          onClick={() => router.push(`/hackathons/${hackathonId}/project/${project.id}`)}
        >
          ...
          <div className="p-6 flex flex-col h-full">
            <div className="mb-4">
              ...
              <p className="text-emerald-100/70 line-clamp-2 mb-4">{project.description}</p>
              <div className="flex flex-wrap gap-2 mb-4">
                {project.techStack.map((tech) => (
                  <Badge key={tech} className="bg-emerald-500/10 text-emerald-200 border-emerald-500/20">
                    {tech}
                  </Badge>
                ))}
              </div>
            ...
            <div className="mt-auto">
              <p className="text-sm text-emerald-300/80 mb-2">
                <span className="font-medium">Team:</span> {project.members.join(", ")}
              </p>
